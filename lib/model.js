...
export function parseVerdict(value) {
  ...
  const evidence = object(input.evidenceStrength, "evidenceStrength");
  const level = text(evidence.level, "evidenceStrength.level", 20).toLowerCase();
  if (!["weak", "emerging", "moderate", "strong"].includes(level)) throw new Error("evidenceStrength.level is invalid");
  return {
    scoring,
    verdict,
    ...
    evidenceStrength:{
      level,
      score:integer(evidence.score, "evidenceStrength.score", 1, 5),
      rationale:text(evidence.rationale, "evidenceStrength.rationale"),
      biggestGap:text(evidence.biggestGap, "evidenceStrength.biggestGap"),
      wouldChangeVerdict:text(evidence.wouldChangeVerdict, "evidenceStrength.wouldChangeVerdict"),
    },
