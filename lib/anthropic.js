...
export async function generateValidation(answers) {
  ...
  const overview = await request(`${wrapped}\nReturn JSON with keys ideaName, tagline, problem, solution, differentiation. Use short strings.`, parseOverview);
  const assumptions = await request(`${wrapped}\nReturn JSON with assumptions (exactly 4 objects: label, assumption, risk low|medium|high, evidence) and validationSteps (exactly 4 objects: label, description, effort, priority high|medium).`, parseAssumptions);
  const verdict = await request(`${wrapped}\nReturn JSON with scoring (exactly 6 objects: name, integer score 1-10, note; names exactly Problem clarity, Market size, Differentiation, Technical feasibility, Monetisation fit, Speed to test), verdict GO|TEST|KILL, integer confidence 1-100, rationale, and nextSteps (exactly 5 strings).`, parseVerdict);
  const calls = [overview, assumptions, verdict];
  return {
