...
export async function ensureSubmissionsTable() {
  if (!schemaReady) {
    const sql = getSql();
    schemaReady = sql`SELECT to_regclass('public.submissions') AS submissions, to_regclass('public.rate_limits') AS rate_limits`.then(rows => {
      if (!rows[0]?.submissions || !rows[0]?.rate_limits) throw new Error("Database migrations have not been applied");
    }).catch(error => {
      schemaReady = null;
      throw error;
    });
